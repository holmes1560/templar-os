...
const CreateCertificationSchema = z.object({
  ...
  credentialUrl: z.string().trim().url().or(z.literal("")).optional(),
...
export async function POST(req: Request) {
  ...
  const item = await db.certification.create({ data: parsed.data });

  await recordRevision({
    entityType: "certification",
    entityId: item.id,
    changeType: "CREATE",
    summary: `Created certification: ${item.name} (${item.issuer})`,
    newValue: item,
